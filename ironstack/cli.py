...
class IronStackCLI:
        ...
        parser = argparse.ArgumentParser(
            ...
            epilog=textwrap.dedent(f"""
                {Colors.CYAN}Examples:{Colors.RESET}
                  ...
                  {Colors.WHITE}ironstack waf start --port 8080{Colors.RESET}
    ...
    def handle_waf(self, args):
        ...
        if args.waf_command == "start":
            print(colored(f"\n🛡️ Starting WAF on port {args.port}...", Colors.BLUE))
            waf = WAF(mode=args.mode)
            waf.start(port=args.port)
            print(colored(f"✅ WAF running on http://127.0.0.1:{args.port}", Colors.GREEN))
